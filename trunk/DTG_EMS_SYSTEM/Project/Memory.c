...
//     NAND_ADDRESS NAND_PAGE;
	...
	memcpy(&NAND_Data_Transmit,Buffer,sizeof(DATA_Transmit));	
     ...
     if(NAND_Data_Transmit.Verify != I2C_EEPROM_VERIFY2)            // 처음라면 리셋 시켜준다.
     {
    	...
    	I2C_EE_BufferRead(Buffer, I2C_EEPROM_WriteAddress0 , sizeof(I2C_Ems_Accure) ); 	
	memcpy(&I2C_Ems_Accure,Buffer,sizeof(I2C_Ems_Accure));	
     ...
     {                    
          ...
          I2C_EE_BufferWrite(Buffer, I2C_EEPROM_WriteAddress0 , sizeof(I2C_Ems_Accure)  );
     }
     ...
     Info.Accident_WritePoint = I2C_Ems_Accure.Accident_WritePoint;
     
     DTG_EMS_DataLoad( NAND_Header_Index.Page_Count - 1);  
     
     DataSize = sizeof(EMS_DRIVE_VALUE);
     
     memcpy(&EMS_Drive_Value, NAND_Buffer,sizeof(EMS_DRIVE_VALUE));
    
     memcpy(&ACC_Drive_Value, &NAND_Buffer[DataSize],sizeof(ACC_DRIVE_VALUE)); 
     
     // [V000031] 누적데이터가 깨질경우 클리어 추가 --------------------------
     
     if(EMS_Drive_Value.DataValid != 1) 
          memset(&EMS_Drive_Value, 0,sizeof(EMS_DRIVE_VALUE));
     
     if(ACC_Drive_Value.DataValid != 1)
...
void Write_Header_EMS(void)           // 인덱스를 저장한다.
{
     ...
     I2C_Ems_Accure.Accident_WritePoint = Info.Accident_WritePoint ;
     
//     I2C_Ems_Accure.Verify = I2C_EEPROM_VERIFY0;                           // 누적 운행거리 저장한다.
     memcpy(&Buffer,&I2C_Ems_Accure,sizeof(I2C_Ems_Accure));	     
     I2C_EE_BufferWrite(Buffer, I2C_EEPROM_WriteAddress0 , sizeof(I2C_Ems_Accure) ); 	      
}
...
//     MemoryTime = MS_TIMER;     
	...
	if( NAND_PAGE.Page == 0 )
	{
     ...
     EMS_Drive_Value.DataValid = 1;
     
     memcpy( &NAND_Buffer , &EMS_Drive_Value , EMS_SIZE );
     
     ACC_Drive_Value.DataValid = 1;
     
     memcpy( &NAND_Buffer[DataSize2] , &ACC_Drive_Value , sizeof(ACC_DRIVE_VALUE));
     
     DataSize2+= sizeof(ACC_DRIVE_VALUE);
          
     //-------------------------------------------------------------------------
     for ( i = 0 ; i < DataCount ; i++)
     {
	     memcpy( &NAND_Buffer[ ( DataSize2 + ( i * DataSize ) ) ], &DTG_NAND_Binary[i], DataSize );
     }
...
{	
     ...
     if(OldSec != SYSTEM_TIME.tm_sec && (Info.Input_Status& INPUT_KEYON_FLAG) )
     {
