...
int USBH_USR_MSC_Application(void)
{
     ...
     switch(USBH_USR_ApplicationState)
     {
          ...
          case USH_USR_FS_IDLE:
          {
               ...
               if( USBH_USR_ActionFlag == USH_USR_FS_READACCIDENT )      // 충돌이벤트 기록하기
               {
                    USBH_USR_ApplicationState = USH_USR_FS_READACCIDENT;
                    USBH_USR_ActionFlag = USH_USR_FS_IDLE;                    
               }                          
          }break;
...
//                    f_stat(FileName,&finfo);
               ...
               {
                    sprintf((char *) FileName, "0:\\EMS_File\\%04d%02d%02d.txt", SYSTEM_TIME.tm_year,SYSTEM_TIME.tm_mon,SYSTEM_TIME.tm_mday);
                    if(! HCD_IsDeviceConnected(&USB_OTG_Core) )
                    {
                         USBH_USR_ApplicationState = USH_USR_FS_IDLE;
                         break;
          ...
          case USH_USR_FS_READACCIDENT:
          {

               u8 Time[13] = {0,};
               for( i2 = 0 ; i2 < Info.AccidentCount ; i2++)
               {
                    Accident_DataLoad(i2); 
                    memcpy(Time,NAND_Buffer,13);
                    sprintf((char *) FileName, "0:\\EVENT_File\\Bk%s.evt",Time   );
                    if(NAND_Buffer[8190] != 0xAA || NAND_Buffer[8191] != 0xAA )
                         break;
                    if(! HCD_IsDeviceConnected(&USB_OTG_Core) )
                    {
                         USBH_USR_ApplicationState = USH_USR_FS_IDLE;
...
//                         bytesToWrite = strlen(( char * )USBH_USR_DTG_Buffer); 
          ...
          case USH_USR_FS_WRITEBACKUP:
          {
               ...
               for( i = 0 ; i < VIEW_Header.Search_Count ; i++)
               {
                    ...
                    if(VIEW_Header.Header.Start_Page == VIEW_Header.Header.End_Page )
                         continue;
                    
                    sprintf((char *) FileName, "0:\\BACKUP_File\\%02d%02d%02d.bin",
     ...
     case USH_USR_FS_READPARAMETER:          // 파라미터 읽기
          {
               ...
               // [V000032] 파라미터 일정길이 이상 못읽는 증상 해결 ---------------------------
