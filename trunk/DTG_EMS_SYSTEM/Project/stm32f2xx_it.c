...
u32 RPM_PWM_Count;
u32 RPM_Count;
u32 RPM_Buffer[MAX_RPM_BUFFERCOUNT];
u8 RPM_Loop;

u32 SPEED_PWM_Count;
u32 SPEED_Count;
u32 SPEED_Buffer[MAX_SPEED_BUFFERCOUNT];
u8 SPEED_Loop;
u8 SPEED_Check;
...
void EXTI9_5_IRQHandler(void)
{
     ...
     SPEED_Loop = SPEED_Loop % MAX_SPEED_BUFFERCOUNT;   // 원형버퍼에 카운터 처리    
     SPEED_Check = 1;
     //////////////////////////////////////////////////
    
     ACC_Drive_Value.Total_Drive_PulsCount1++;           // 저장용 펄스 계산
...
//          RPM_PWM_Count = 0;     //  초기화
          ...
          SPEED_Buffer[SPEED_Count++ ] = ( SPEED_PWM_Count * 1000)  ;  // / 2.5 (나누기 2.5)
	...
	if(EXTI_GetFlagStatus(EXTI_Line15))
     {
          ...
          RPM_Loop = RPM_Loop % MAX_RPM_BUFFERCOUNT;   // 원형버퍼에 카운터 처리      
