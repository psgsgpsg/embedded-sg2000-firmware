...
u8 EMS_Setting( u32 PageNum )
{
     ...
     ACC_DRIVE_VALUE ACC_Drive_Temp;
     ...
     {     
          ...
          {
               PageTemp = PageNum + ( ( i + 1 ) * DATA_SKIPCOUNT );        // 
               
               if(NAND_Data_Transmit.End_Page < PageTemp )            // 
                    break;
               
               DTG_EMS_DataLoad( PageTemp -1);
               
               DataSize = sizeof(DTG_Binary);  
               DataSize2 = EMS_SIZE + sizeof(ACC_Drive_Value);
               memcpy( &DTG_Binary, &NAND_Buffer[ ( DataSize2 + ( i * DataSize ) ) ], DataSize );
               ...
               memcpy( &ACC_Drive_Temp, &NAND_Buffer[EMS_SIZE], sizeof(ACC_Drive_Temp) );               
               ...
               ConvertString(TLV_BUFFER.Data10, EMS_Drive_Temp.EcoUse_Time );                  // 고RPM 시간         
...
u8 KMap_Setting(u32 PageNum)
{
     ...
     if(KMap_Transmit_Count == 0 )    
     {
          ...
          {              
               ...
               DataSize2 = EMS_SIZE + sizeof(ACC_Drive_Temp);
               memcpy( &DTG_Binary, &NAND_Buffer[ ( DataSize2 + ( i * DataSize ) ) ], DataSize );
               if(DTG_Binary.DataValid == 1)
               {
                    RTC_BinaryToDate(DTG_Binary.Datetime,&datetime);
...
void Wifi_Process(void)
{
     ...
     if(SFLASH_Memory.DTG_EMS_Select == 0)   // DTG      
     {
          if( ( NAND_Data_Transmit.Start_Page ) > NAND_Data_Transmit.End_Page)     // 보낼 페이지가 없으면
               return;          
     }else
     {    
          if( ( NAND_Data_Transmit.Start_Page + DATA_SKIPCOUNT ) > NAND_Data_Transmit.End_Page)     // 보낼 페이지가 없으면
           return;
     }
     WaitTimeCount++;
               ...
               else          //접속 완료 상태 데이터를 전송한다.
               {
                    if(NAND_Data_Transmit.Start_Page < NAND_Data_Transmit.End_Page)
                    {
                         ...
                         if( Wifi_Send_Result != WIFI_WAIT && Wifi_Send_Result != WIFI_SEND)
                         {
                              ...
                              if(SFLASH_Memory.DTG_EMS_Select == 0)   // DTG 
                              {    
                                   KMap_Setting( NAND_Data_Transmit.Start_Page );  
                                   Wifi_Send_Result = WIFI_SEND;
                              }
                              else // EMS
                              {                                   
                                   if(NAND_Data_Transmit.End_Page - NAND_Data_Transmit.Start_Page >= DATA_SKIPCOUNT)     
                                   {
                                        EMS_Setting( NAND_Data_Transmit.Start_Page);
                                        Wifi_Send_Result = WIFI_SEND;
                                   }
                              }                              
                         }
                         ...
                         if(Protocol_Rev.ServiceID[2] == 0x0f && Protocol_Rev.ServiceID[3] == 0xa1)
                         {
                              if(Protocol_Rev.Message.Parameter4.Name[0] == 'R' && Protocol_Rev.Message.Parameter4.Name[2] == 'S')
                              {
                                   ...
                                   if(Answer == 0 || Answer == 101)
                                   {
                                        ...
                                        Write_Transmit_Header();      // 전송하고 저장하기

                                        if(SFLASH_Memory.DTG_EMS_Select == 0)   // DTG      
                                        {
                                             NAND_Data_Transmit.Start_Page++;
                                             if( NAND_Data_Transmit.End_Page == NAND_Data_Transmit.Start_Page ) //전송할 페이지가 없으면 종료
                                             {
                                                  Wifi_Result = WIFI_NO_INIT;
                                                  Wifi_Reset();                                             
                                             }                                             
                                        }else
                                        {
                                             ...
                                             NAND_Data_Transmit.Start_Page += ( Trans_PageCount * DATA_SKIPCOUNT );
                                             
                                             if( NAND_Data_Transmit.End_Page - NAND_Data_Transmit.Start_Page < DATA_SKIPCOUNT) //전송할 페이지가 없으면 종료
                                             {
                                                  Wifi_Result = WIFI_NO_INIT;
                                                  Wifi_Reset();
                                             }
