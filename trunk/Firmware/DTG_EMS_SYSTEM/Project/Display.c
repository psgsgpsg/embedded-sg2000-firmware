...
void Display(void)
{
	...
	switch(DisplayInfo.Now_View)
	{
     case DISP_MENU_USECOM:
          {
			sprintf( (char *)Display_Buff," COMMNUNICATION MODE  " );
			GLcd_PutString(0,0,Display_Buff,1);
			sprintf( (char *)Display_Buff,"  CONNECT USE CABLE   " );		        
			GLcd_PutString(0,16,Display_Buff,1);
     ...
     case DISP_STARTMOTOR_VIEW:         // 스타트 모터 뷰어
          {
			...
			GLcd_PutString(0,0,Display_Buff,1);
			sprintf( (char *)Display_Buff,"     [ %06d ]   ",ACC_Drive_Value.Total_StartMotor);	        
			GLcd_PutString(0,16,Display_Buff,1);                
          }break;
	...
	case DISP_MENU4_0:
		{
			...
			GLcd_PutString(0,0,Display_Buff,1);
               ...
               for( i =0  ; i < Pass_Index ; i++)
                   PasswordBuffer[i] = '*';
			sprintf( (char *)Display_Buff,"      [ %4s ]  ",PasswordBuffer);
			GLcd_PutString(0,16,Display_Buff,1);	
			DisplayInfo.Mode = DISPMODE_MENU;	
	...
	case DISP_MENU4_2:
		{
			...
			GLcd_PutString(0,0,Display_Buff,1);             
               ...
               if( SFLASH_Memory.Car_Maker == 1 )
                    sprintf( (char *)Display_Buff,"  현대2006-1     ");
               if( SFLASH_Memory.Car_Maker == 2 )
                    sprintf( (char *)Display_Buff,"  현대2007-2      ");
               if( SFLASH_Memory.Car_Maker == 3 )
                    sprintf( (char *)Display_Buff,"  현대2011-3     ");
               if( SFLASH_Memory.Car_Maker == 4 )
                    sprintf( (char *)Display_Buff,"  형대2011저상-4   ");
               if( SFLASH_Memory.Car_Maker == 5 )
                    sprintf( (char *)Display_Buff,"  현대구형저상-5  ");
               if( SFLASH_Memory.Car_Maker == 6 )
                    sprintf( (char *)Display_Buff,"  현대신형-6       ");
               if( SFLASH_Memory.Car_Maker == 7 ) 
                    sprintf( (char *)Display_Buff,"  현대신형저상-7   ");
               if( SFLASH_Memory.Car_Maker == 8 )
                    sprintf( (char *)Display_Buff,"  대우구형-8       ");
               if( SFLASH_Memory.Car_Maker == 9 )
                    sprintf( (char *)Display_Buff,"  대우구형저상-9   ");
               if( SFLASH_Memory.Car_Maker == 10 )
                    sprintf( (char *)Display_Buff,"  대우신형-10      ");
               if( SFLASH_Memory.Car_Maker == 11 )
                    sprintf( (char *)Display_Buff,"  대우신형초저상-11  ");
               if( SFLASH_Memory.Car_Maker == 12 )
                    sprintf( (char *)Display_Buff,"  글로벌시티-12    ");
               if( SFLASH_Memory.Car_Maker == 13 )
                    sprintf( (char *)Display_Buff,"  카운티구형-13    ");
               if( SFLASH_Memory.Car_Maker == 14 )
                    sprintf( (char *)Display_Buff,"  카운티신형-14      ");
			GLcd_PutString(0,16,Display_Buff,1);	
			DisplayInfo.Mode = DISPMODE_MENU;	
