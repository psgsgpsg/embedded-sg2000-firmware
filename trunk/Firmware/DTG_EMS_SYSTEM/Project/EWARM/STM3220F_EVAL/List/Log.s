...
//    Cpu mode     =  thumb                                                   /
//    Endian       =  little                                                  /
//    Source file  =  F:\¹¬ÀÛ¾÷\[ NewDTG ]\WORK\DTG_EMS_SYSTEM\Project\Log.c  /
//    Command line =  "F:\¹¬ÀÛ¾÷\[ NewDTG ]\WORK\DTG_EMS_SYSTEM\Project\Log.c /
...
// F:\¹¬ÀÛ¾÷\[ NewDTG ]\WORK\DTG_EMS_SYSTEM\Project\Log.c
//    1 #include "Log.h"
//    2 #include "main.h"
...
//   10      LOGWRITE = LOG_BASIC | LOG_MEMORY | LOG_EEPROM |LOG_TILT| LOG_EMS_ENGINE | LOG_USB_SYSTEM;    // ·Î±× ±â·Ï ¿©ºÎ
Log_Config:
        LDR.N    R0,??DataTable2
        MOVW     R1,#+5425
        STR      R1,[R0, #+0]
...
//   24              string);
        ...
        BL       sprintf
//   25      
//   26      if( Device & UART_OUT == UART_OUT )   // Åë½ÅÆ÷Æ®·Î Àü¼Û
        LDR.N    R0,??DataTable2_1
        LDRB     R0,[R0, #+0]
        LSLS     R0,R0,#+31
        BPL.N    ??Write_Log_0
...
//   29      }     
//   30 
//   31      if( Device & USB_OUT == USB_OUT)     // USB·Î Àü¼Û
??Write_Log_0:
        LDR.N    R0,??DataTable2_1
        LDRB     R0,[R0, #+0]
        LSLS     R0,R0,#+31
        BPL.N    ??Write_Log_1
//   32      {
//   33           if( USBH_USR_ApplicationState == USH_USR_FS_IDLE)      // USB°¡ ÁØºñ°¡ µÇ¾ú´Ù¸é Ãâ·Â
