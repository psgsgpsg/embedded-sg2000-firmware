...
//                    _USB_Device_Library\Core\src\usbd_req.c                 /
//    Command line =  F:\Work\S&G2000\Firmware\DTG_EMS_SYSTEM\Libraries\STM32 /
//                    _USB_Device_Library\Core\src\usbd_req.c -D              /
...
// F:\Work\S&G2000\Firmware\DTG_EMS_SYSTEM\Libraries\STM32_USB_Device_Library\Core\src\usbd_req.c
//    1 /**
//    2   ******************************************************************************
//    3   * @file    usbd_req.c
...
??USBD_GetDescriptor_13:
        ADD      R2,SP,#+0
        MOVS     R1,R6
        ADR.W    R0,`?<Constant "STM32 Virtual ComPort...">`
        BL       USBD_GetString
        B.N      ??USBD_GetDescriptor_19
??USBD_GetDescriptor_16:
        ADD      R2,SP,#+0
        MOVS     R1,R6
        ADR.W    R0,`?<Constant "00000000050C">`
        BL       USBD_GetString
        B.N      ??USBD_GetDescriptor_19
??USBD_GetDescriptor_15:
        ADD      R2,SP,#+0
        MOVS     R1,R6
        ADR.W    R0,`?<Constant "VCP Config">`
        BL       USBD_GetString
        B.N      ??USBD_GetDescriptor_19
??USBD_GetDescriptor_17:
        ADD      R2,SP,#+0
        MOVS     R1,R6
        ADR.W    R0,`?<Constant "VCP Interface">`
        BL       USBD_GetString
        B.N      ??USBD_GetDescriptor_19
...
`?<Constant "STMicroelectronics">`:
        ...
        SECTION `.text`:CODE:NOROOT(2)
        DATA
`?<Constant "STM32 Virtual ComPort...">`:
        ; Initializer data, 36 bytes
        DC8 83, 84, 77, 51, 50, 32, 86, 105, 114, 116
        DC8 117, 97, 108, 32, 67, 111, 109, 80, 111, 114
        DC8 116, 32, 32, 105, 110, 32, 70, 83, 32, 77
        DC8 111, 100, 101, 0, 0, 0

        SECTION `.text`:CODE:NOROOT(2)
        DATA
`?<Constant "00000000050C">`:
        ; Initializer data, 16 bytes
        DC8 48, 48, 48, 48, 48, 48, 48, 48, 48, 53
        DC8 48, 67, 0, 0, 0, 0
...
`?<Constant "VCP Config">`:
        ...
        DC8 86, 67, 80, 32, 67, 111, 110, 102, 105, 103
        DC8 0, 0

        SECTION `.text`:CODE:NOROOT(2)
        DATA
`?<Constant "VCP Interface">`:
        ; Initializer data, 16 bytes
        DC8 86, 67, 80, 32, 73, 110, 116, 101, 114, 102
        DC8 97, 99, 101, 0, 0, 0
