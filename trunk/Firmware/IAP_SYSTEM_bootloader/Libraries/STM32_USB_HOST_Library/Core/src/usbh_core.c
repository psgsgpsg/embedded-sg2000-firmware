...
//    phost->usr_cb->DeviceAttached(); 디스플레이
    pdev->host.port_cb->DisconnStatus = 0; 
    ...
    if ( HCD_ResetPort(pdev) == 0)
    {
//      phost->usr_cb->ResetDevice();    Call Back
      /*  Wait for USB USBH_ISR_PrtEnDisableChange()  
      Host is Now ready to start the Enumeration 
      */
      
      phost->device_prop.speed = HCD_GetCurrentSpeed(pdev);   // 스피드 레지스터에서 읽기
      
      phost->gState = HOST_ENUMERATION;
      phost->usr_cb->DeviceSpeedDetected(phost->device_prop.speed);   //  스피드 설정
...
//    phost->usr_cb->DeviceDisconnected();    디스플레이
