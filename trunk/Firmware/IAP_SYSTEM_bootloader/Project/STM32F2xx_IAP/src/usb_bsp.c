...
#define STM322xG_EVAL_RevB 
//#define STM3220F_EVAL_RevA 
/**
* @}
*/ 
...
void USB_OTG_BSP_Init(USB_OTG_CORE_HANDLE *pdev)
{
	...
	GPIO_InitTypeDef GPIO_InitStructure;
			...
			/* Configure INTN SCL SDA (Phy/I2C) Pins */
			...
			GPIO_PinAFConfig(GPIOB,GPIO_PinSource6,GPIO_AF_OTG1_FS) ; 
			GPIO_PinAFConfig(GPIOB,GPIO_PinSource8,GPIO_AF_OTG1_FS) ; 
			GPIO_PinAFConfig(GPIOB,GPIO_PinSource9,GPIO_AF_OTG1_FS) ;  
		#else  
			RCC_AHB1PeriphClockCmd( RCC_AHB1Periph_GPIOA , ENABLE);  	
			/* Configure SOF VBUS ID DM DP Pins */
			GPIO_InitStructure.GPIO_Pin = //GPIO_Pin_8  | 	// ID
			//	GPIO_Pin_9  | 		// VBUS
				GPIO_Pin_11 | 		// DM
				GPIO_Pin_12;		// DP
...
//			GPIO_PinAFConfig(GPIOA,GPIO_PinSource10,GPIO_AF_OTG1_FS) ;   
		...
		#endif  	// End of USE_I2C_FS_PHY	
		RCC_APB2PeriphClockCmd(RCC_APB2Periph_SYSCFG, ENABLE);
		
		RCC_AHB2PeriphClockCmd(RCC_AHB2Periph_OTG_FS, ENABLE) ; 
		
	#else // USE_USB_OTG_HS 일 경우
	...
	// Delay(설정) //타이머를 사용하기 위해서는 Define 을 USE_ACCURATE_TIME 
	// 정확한 타이머가 필요하다면 
...
#if defined (STM3220F_EVAL_RevA) && defined (USE_STM322xG_EVAL)  
	if (1 == state)
#else
		if (0 == state)
#endif  
