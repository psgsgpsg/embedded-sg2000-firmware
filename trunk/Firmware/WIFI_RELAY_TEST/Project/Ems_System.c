...
void Emergency(void)
{
     ...
     }else          // 출력이 나가고 있다면 3초간 대기
     {
          ...
          if(CheckTimeOver(5000, EMS_Emc_Timer))            // 3초간 출력이 나가고 있다면 강제 클리어
          {
               RESET_ENGINE_ON();
               Check = 0;
          }
...
void EMS_Watching(void)
{
     ...
     if ( !(Info.Input_Status & INPUT_PARKING_BREAK_FLAG ))
          if( Info.SPEED_Value > 1 )	// 1km 이상 속도가
               if( Flag.RPM1Min == 0 )  // RPM이 나타난지 1초 이후라면
                    EnginOffLimit_Flag = 0;       // 3초
     
     if(!CheckTimeOver(1000, EMS_WATCH_Timer2))return;  //1000ms 에 한번씩 EMS를 동작 스텝을 기록한다..                  
     
     EMS_WATCH_Timer2 = MS_TIMER;
     
     // LOG -------------------------------------------------------------------------
     // 로그 기록단위 500ms 
     if(LOGWRITE & LOG_EMS_ENGINE)                       
     {
          sprintf((char*)Buffer,"EMS Check-%d / %d [%04x] \r\n ",EMSOn_Check,EMSOff_Check,INPUT_DATA); 
          Write_Log(Buffer, NORMAL_LOG);
     ...
     case 0x03:
          {
               ...
               {
                    if(Info.RPM_Value > 400)    //rpm이 0이 될때 까지 기둘린다.                    
                    {
