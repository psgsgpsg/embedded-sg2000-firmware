...
void UART4_IRQHandler(void)		// GPS 데이터를 받는다.
{
	...
	if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)
	{
		...
		if (USART_GetFlagStatus(UART4, USART_FLAG_NE) != RESET) {
			USART_ClearITPendingBit(UART4, USART_FLAG_NE);
               //			ErrorFlag = 1;
		}
		if (USART_GetFlagStatus(UART4, USART_FLAG_FE) != RESET) {
			USART_ClearITPendingBit(UART4, USART_FLAG_FE);
               //			ErrorFlag = 1;
		}
		if (USART_GetFlagStatus(UART4, USART_FLAG_PE) != RESET) {
			USART_ClearITPendingBit(UART4, USART_FLAG_PE);
               //			ErrorFlag = 1;
		}
		...
		*/
		...
		// Clear the USART2 Receive interrupt 
		USART_ClearITPendingBit(UART4, USART_IT_RXNE | USART_IT_TXE | USART_IT_ERR);
	} 
}
