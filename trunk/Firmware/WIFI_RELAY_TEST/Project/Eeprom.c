...
//			if(LOGWRITE & LOG_EEPROM)                       
//			{
//				sprintf((char *)Buffer,"SFLASH VERIFY ERROR Count = %d\r\n ", ReWriteCount ); 
...
//     }
     ...
     if(SFLASH_Memory.Car_Maker == 2)        // 대우신형
     {
          Set_Info.Speed_OUT = 1;     
     }
     else
	...
	if( SFLASH_CarInfo.Verify != SFLASH_VERIFY0_CARINFO )        // Verify를 읽었을때 오류인가
     {
          ...
          sprintf( (char *) SFLASH_CarInfo.SerialNumber, "1X21WK11000000" );
          sprintf( (char *) SFLASH_CarInfo.MachineVer, "DTGEMS00001" );
          sprintf( (char *) SFLASH_CarInfo.Recognition, "11-0033-01" );
          sprintf( (char *) SFLASH_CarInfo.CarNum, "경기11일1234" );
...
//	printf("%x %x ",SFLASH_Memory.Verify,SFLASH_VERIFY1);
	if( SFLASH_Memory.Verify != SFLASH_VERIFY1_MEMORY )        // Verify를 읽었을때 오류인가?
	{
          ...
          SFLASH_Memory.EMS.TILTY_Limit = 3;
          ...
          SFLASH_Memory.Voice_Volume = 100;
          ...
          SFLASH_Memory.GPS_EMS_Use = 0;               // GPS EMS 사용 0은 사용하지 않음(전체) 1: 차고지만 사용 2: 구간 EMS 사용         
