...
void TIM4_Config(void)			// 대우 구형 타이머 // 사용하지 않음
{
     ...
     TIM_TimeBaseStructure.TIM_Period = 99 ; // 오버플로워 한계값 1000;    // 1000us 1ms
...
//     if( SPEED_Count > 0 )          // 들어온 카운트가 1hz 보다 낮으면 클리어
//     {
//          for( i = 0 ; i < MAX_RPMSPEED_BUFFERCOUNT ; i++)
//          {
//               SPEED_Buffer[i] = 0;
...
//          Speed_Avr = 0;
//     }
     if(SPEED_Check == 0)         // 들어온 펄스가 한개도 없다면
     {
          if(Low_Count++ > 10)          // 1초동안 검사
          {
               for( i = 0 ; i < MAX_RPMSPEED_BUFFERCOUNT ; i++)
               {
                    SPEED_Buffer[i] = 0;
     ...
     if( ( Set_Info.Speed_OUT == 1 ) && ( error_flg == 1 ) )                        // SPEED출력일경우 1.4는 상수
     {    
          Speed_Hz = 200000 / Speed_Avr * 2;
          Speed_Hz /= 1.481;
          Speed_Change = 5000 / Speed_Hz;
     ...
     if( error_flg == 1 )
     {
          ...
          SpeedTmp = (unsigned int)( Set_Info.SPEED_Adjust * 2000) / Speed_Avr;   
          if(SpeedTmp > 255)
...
//     static u16 Accident_Cnt2 = 0;
     ...
     Info.EMS_Data.GPS_Speed = GPSInfo.Conv.GPS_Speed;
     ...
     if( Info.Total_Drive_PulsCount1 > PLUS_DIVIDE)
     {
         Info.Total_Drive_PulsCount1 -= PLUS_DIVIDE;
         Info.Total_Drive_PulsCount2++;
     }     
     Info.TotalDriveLength = ((Info.Total_Drive_PulsCount1 * Set_Info.Distance_Adjust) + 
                              (Info.Total_Drive_PulsCount2 * PLUS_DIVIDE) * Set_Info.Distance_Adjust ) / 1000 ;      
...
//          WWDG_SetCounter(127);
          ...
          {
               SettingWIFI("118.107.161.70,9513,TAEHO,taeho9901");
               First_Setting = 1;
          }
          ...
          // --------- EMS ----------------------------------------------------
          if(SFLASH_Memory.EMS.USE_Enable)         // EMS를 사용할지 확인한다. 
          {
