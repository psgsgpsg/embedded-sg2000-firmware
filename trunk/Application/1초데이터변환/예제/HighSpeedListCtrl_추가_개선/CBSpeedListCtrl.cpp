...
BOOL CCBSpeedListCtrl::SetItemText(int nItem, int nSubItem, LPCTSTR lpszText)
{
	...
	m_arLabels[nItem].m_strText2 = lpszText;
...
void CCBSpeedListCtrl::OnGetdispinfo(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (pItem->mask & LVIF_TEXT)
	{
		...
		switch(pItem->iSubItem)
		{
			...
			case 1:
				//sprintf(pItem->pszText,"xxx%s",rLabel.m_strText2);
					lstrcpy(pItem->pszText, rLabel.m_strText2);
				break;
		}
	}
...
//        return strcmp(p1->sItem[compare_column],p2->sItem[compare_column]);
		return strcmp(p1->m_strText,p2->m_strText);
		return 0;
}
...
int __cdecl CompareByLabelNameD(const void *elem1, const void *elem2)
{
      ...
      //  return strcmp(p2->sItem[compare_column], p1->sItem[compare_column]);
		return strcmp(p2->m_strText,p1->m_strText);
		return 0;
}
